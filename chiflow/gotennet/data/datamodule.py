...
class RDB7Preparation(BaseDatasetPreparation):
    """Dataset preparation for RDB7."""

    def load_dataset(self, root: str, dataset_arg: str) -> QM9:
        transform = normalize_positions if self.hparams["normalize_positions"] is None else None
        if transform:
            log.warning("Normalizing positions for RDB7 dataset.")
        return RDB7(root=root, dataset_arg=dataset_arg, transform=transform)
...
class DataModule(LightningDataModule):
    ...
    def __init__(self, hparams: Union[Dict, "DictConfig"]):
        ...
        self.dataset_preparations = {
            ...
            "RDB7": RDB7Preparation(self.hparams)
        }
