...
from flow_matching.utils import (
    rmsd_loss,
    compute_steric_clash_penalty,
    pred_atom_index_align,
    calc_DMAE,
    pred_atom_index_align_mad,
)
from flow_matching.chirality_metrics import (
    evaluate_chirality_correctness,
    evaluate_persistent_chirality_correctness,
)
...
def evaluate_geometry(
    ...
    """
    Evaluate the geometric quality of generated molecular conformations.
    
    This function computes various geometric metrics to assess the quality of
    generated molecular structures compared to ground truth, including RMSE,
    MAE, bond angles, dihedral angles, steric clashes, and chirality correctness.
    
    Args:
        data (Data): PyTorch Geometric data object containing molecular information
        r_threshold (float): Distance threshold for steric clash penalty. Defaults to 1.2.
        epsilon (float): Scaling factor for the clash penalty. Defaults to 1.0.

    Returns:
        Dict[str, float]: Dictionary containing geometric evaluation metrics:
            - mae: Mean absolute error of distances
            - rmse: Root mean square error of atomic positions
            - angle_error: Bond angle error in degrees
            - dihedral_error: Dihedral angle error in degrees  
            - steric_clash: Steric clash penalty difference
            - Plus chirality correctness metrics
    """
    rtsp_i = 1
    # RMSE error
    pred_pos_N_3, gt_pos_N_3 = pred_atom_index_align(
        data.smiles, data.pos[:, rtsp_i, :], data.pos_gen[-1]
    )
    rmse = rmsd_loss(pred_pos_N_3, gt_pos_N_3)
    # rmse_median = rmsd_median_loss(pred_pos_N_3, gt_pos_N_3)
    # rmse_raw = rmsd_loss(data.pos_gen[-1], data.pos[:, rtsp_i, :])
    # MAE error
    pred_pos_aligned_mae = pred_atom_index_align_mad(
        data.smiles, data.pos[:, rtsp_i, :], data.pos_gen[-1]
    )
    mae = calc_DMAE(
        cdist(gt_pos_N_3, gt_pos_N_3), cdist(pred_pos_aligned_mae, pred_pos_aligned_mae)
    )
    ...
    bond_angle_error = (
        torch.abs(gt_angles_M1 - pred_angles_M1) * 180.0 / math.pi
    ).mean()
    ...
    # LJ potential (for fun)
    """pred_mol = create_ase_molecule(data.atom_type.cpu().numpy(), pred_pos_N_3.cpu().numpy())
    gt_mol = create_ase_molecule(data.atom_type.cpu().numpy(), gt_pos_N_3.cpu().numpy())
    pred_pot = calculate_pot_energy(pred_mol)
    gt_pot = calculate_pot_energy(gt_mol)
    pot_diff = pred_pot - gt_pot"""

    # Chirality evaluation - use the TS positions (ground truth)
    chirality_metrics = evaluate_chirality_correctness(data.pos[:, rtsp_i, :], data)

    # Persistent chirality evaluation - only for chiral centers that stay the same
    persistent_chirality_metrics = evaluate_persistent_chirality_correctness(
        data.pos[:, rtsp_i, :], data
    )
    ...
    # Add chirality metrics
    result.update(chirality_metrics)
    result.update(persistent_chirality_metrics)
...
class TestAndSaveResultsAfterTrainingCallback(L.Callback):
    ...
    def save_stats_to_csv(self, pd_results_mean, module):
        ...
        if mr_stats_file.exists():
            ...
            pd.DataFrame([pd_results_mean]).to_csv(
                mr_stats_file, index=False, float_format="%.3f"
            )
    ...
    def on_test_end(self, trainer: Trainer, module):
        inference_time_per_rxn = (time.perf_counter() - self._test_start_time) / len(
            module.results_R
        )
        self._test_start_time = None
        ...
        pd_results_df.to_csv(
            self.save_path / "test_results.csv", index=False, float_format="%.3f"
        )

        means = pd_results_df.mean()
        means["time"] = inference_time_per_rxn
        # stds = pd_results_df.std()
        # stds['time'] = '-'
        # stats_df = pd.DataFrame([means, stds], index=['mean', 'std'])
        # stats_df.to_csv(self.save_path / 'test_results_stats.csv', float_format='%.3f')
