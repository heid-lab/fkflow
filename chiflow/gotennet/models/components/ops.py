...
def parse_update_info(edge_updates):
    ...
    allowed_parts = ["gated", "gatedt", "norej", "mlp", "mlpa", "act", "linw", "linwa",
                     "drej"]
...
class _VDropout(nn.Module):
    ...
    def forward(self, x, dim=-1):
        ...
        mask = torch.bernoulli(
            (1 - self.drop_rate) * torch.ones(shape, device=device)
        )
        x = mask * x
        if self.scale:
...
# TODO: this is SiLU: replace w. its pytorch module
class swish(nn.Module):
    def __init__(self):
        super().__init__()
...
class EdgeInit(MessagePassing):
    def __init__(self, num_rbf, hidden_channels, activation=F.silu, proj_ln='',
                 last_activation=False, weight_init=nn.init.xavier_uniform_, bias_init=nn.init.zeros_, ):
        ...
        if type(hidden_channels) == int:
            hidden_channels = [hidden_channels]
        self.edge_up = MLP([num_rbf] + hidden_channels, activation=activation, norm=proj_ln, weight_init=weight_init,
                           bias_init=bias_init,
                           last_activation=activation if last_activation else None)
    ...
    def forward(self, edge_index, edge_attr, edge_type_r, edge_type_p):
        """
        Args:
            edge_attr (Tensor): basis expanded lengths
        
        Comment: 
            What is different here (to the original implementation) is that we don't
            incorporate node features (h_i + h_j) * self.edge_up(edge_attr).
