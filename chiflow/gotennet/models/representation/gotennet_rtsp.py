...
class GotenNet(nn.Module):
    def __init__(
        ...
        """
        # RTSP: separate node init embeddings
        self.neighbor_embeddings = nn.ModuleList([
            NodeInit(
                [self.hidden_dim // 2, self.hidden_dim], n_atom_rdkit_feats, n_rbf,
                self.cutoff, max_z=max_z, weight_init=weight_init, bias_init=bias_init, concat=False,
    ...
    # TODO: adapt to rtsp
    def forward(self, x_t_N_3_3: Tensor, t_G: Tensor, inputs: Mapping[str, torch.Tensor]) -> Tuple[
        torch.Tensor, torch.Tensor]:
        """
        Compute atomic representations/embeddings.

        Args:
            x_t_N_3_3 (Tensor): nodes of batch graphs (N), r/ts/p (3), xyz coordinates (3)
            t_G (Tensor): random uniform times (0 to 1) for each graph (G)
        ...
        q = self.neighbor_embeddings(atom_type, r_feat, p_feat, q, edge_index, edge_weight, edge_attr, edge_type_r,
                                    edge_type_p)
        q = q + t_emb_N

        # edge_attr = self.edge_embedding(edge_index, edge_attr, q)
        edge_emb = self.edge_embeddings(edge_index, edge_attr, edge_type_r, edge_type_p)
