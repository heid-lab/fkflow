...
def get_atom_index_by_mapnum(mol, mapnum):
    """
    Get atom index by its atom map number in a molecule.
    
    Args:
        mol (Mol): RDKit molecule object
        mapnum (int): Atom map number to search for
        
    Returns:
        int or None: Atom index if found, None otherwise
    """
...
def unmap_smarts(smarts_str):
    ...
    Args:
        smarts_str (str): SMARTS string with atom map numbers
        
    Returns:
        str: SMARTS string without atom map numbers
        
    Raises:
        ValueError: If SMARTS string cannot be parsed
    """
...
def get_small_rxn_smiles_and_mapnums2remove(r_smiles, p_smiles) -> Tuple[str, List[int]]:
    """
    Generate a simplified reaction SMILES by removing unchanged atoms.
    
    This function identifies atoms that have the same neighbors and bond types
    in both reactants and products, and removes them to create a reaction core
    representation focused on the changing atoms.
    
    Args:
        r_smiles (str): Reactant SMILES string with atom mapping
        p_smiles (str): Product SMILES string with atom mapping
        
    Returns:
        Tuple[str, List[int]]: A tuple containing:
            - Simplified reaction SMILES string
            - List of atom map numbers (0-indexed) that were removed
    """
    params = Chem.SmilesParserParams()
    params.removeHs = False
    params.sanitize = False
